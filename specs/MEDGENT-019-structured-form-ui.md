...
- [x] **HCP Name** — "Search or select HCP..." typeahead against
      `GET /api/v1/hcps` (MEDGENT-006/017). This is the app's only HCP
      picker — there is no separate HCP list/nav screen in this build.
- [x] **Interaction Type** — dropdown (Meeting, Call, Email, Conference,
      etc.).
- [x] **Date** and **Time** — combined into `occurred_at` on submit.
- [x] **Attendees** — free multi-value text input (names).
- [x] **Topics Discussed** — multi-line text area, with a **"Summarize
      ...
      functionality.
- [x] **Materials Shared** — add/search multi-value list, distinct from
      Samples Distributed.
- [x] **Samples Distributed** — add multi-value list, distinct from
      Materials Shared.
- [x] **Observed/Inferred HCP Sentiment** — Positive / Neutral / Negative
      radio group.
- [x] **Outcomes** — multi-line text area.
- [x] **Follow-up Actions** — multi-line text area for free-text notes,
...
- [x] Client-side validation matches backend validation (HCP required,
      valid interaction type, valid sentiment value) with inline error
      messages.
- [x] Submitting calls the Interaction create endpoint via MEDGENT-017's
      RTK Query mutation with `source: "form"`.
- [x] Success state confirms the save; failure state (network/validation
      error) preserves the rep's entered data.
- [x] Render/interaction tests: successful submit, validation error,
